(() => {
  ...
  onReady(async()=>{
    ...
    async function render(){
      ...
      for(const item of items){const f=fm[item.receipt_file_id];const url=f?await signedUrl(f.storage_path):'';cards.push(`<article class="ss-budget-card"><div class="ss-budget-card-top"><div><h4>${esc(item.description)}</h4><div class="ss-budget-meta">${esc(pretty(item.category))}</div></div><span class="ss-budget-badge ${esc(item.status)}">${esc(pretty(item.status))}</span></div><div class="ss-budget-amounts"><div class="ss-budget-amount"><small>Planned</small><strong>${money(item.planned_amount)}</strong></div><div class="ss-budget-amount"><small>Approved</small><strong>${money(item.approved_amount)}</strong></div><div class="ss-budget-amount"><small>Spent</small><strong>${money(item.actual_amount)}</strong></div></div>${url?`<div class="ss-budget-card-actions"><a class="ss-budget-receipt" href="${esc(url)}" target="_blank" rel="noopener">Open Receipt</a></div>`:''}</article>`)}
